...
2. **逐模块分段填充 (Phase alpha/A/B/C)**：对每个模块依次执行以下闭环：
   ...
   **Phase C：达标确认**
   ...
   - 仅当该模块 `fill_ratio >= 1.0` **且** Slide 达底线后标记 `done`
    ...
    - **费曼画板探针 (Feynman Canvas Probe)** 🆕：
      * 对当前模块中每个含 `**加粗锚词**` 的段落，提取加粗词组
      * 对每个加粗词组，执行画面映射："用一个大一新生能在脑海中画出的画面描述这个概念"
      * **验证**：画面描述中是否包含至少一个具体的物理实体（人/物/场所/动作）？
        - ✅ "像在超市只有 5 分钟" → 有物理实体（超市、时钟）→ 通过
        - ❌ "认知维度的向上跃迁" → 无物理实体 → 标记 `[UNPAINTABLE]` 🔴，回退改写该段
