...
5.  **Vibe-ification 概念映射 (Vibe Coding Interceptor)**：如果提取出的教材原文涉及具体编程语法（如 For 循环、DOM 操作、API 参数），**强制将其转换为 Vibe Coding 原语**（详见 `rule_vibe_coding.md` §3）。例如将"状态管理"映射为"要求 AI 记住上下文的 Prompt 策略"。该映射必须写入备忘录，防止在 Phase 2 写作时产生语法级教学。
6.  **产出案例提取清单**（ADR 023 强制）：以下表格形式列出教材中可用于本单元的所有**具体案例/实验/设计启示/design implications**：
...
7.  **教材图覆盖率预检**（视觉资产前置发现）：在产出案例提取清单的同时，扫描教材相关章节中的**所有 `![](images/...)` 引用**。
    对每张教材图评估：是否可作为本单元某个 `[VISUAL]` 块的候选？
    将候选教材图追加到案例提取清单中，标注 `asset_type: image`：

    | 来源章节 | 图注/Figure | 可映射的 VISUAL 场景 | asset_type |
    |---|---|---|---|

    > **⚠️ 零匹配报告强制**：即使教材章节中无可用图片，也必须显式报告 `教材图候选: 0`，不得静默跳过。
