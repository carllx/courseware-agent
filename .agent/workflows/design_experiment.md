...
4. **强制拦截与数据补全**：强制校验 `exp_X.yaml` 必须包含 `steps` 数组，且每个 step 必须包含 `guide_text`。如果缺失，**Agent 必须主动介入**：结合课程上下文草拟步骤并向用户确认，或直接要求用户提供，严禁在缺失该字段时向下流转调用 `generate_exp_docs.py`。
...
1. `Exp_<编号>_<名称>_实验指导书.md` — 含 4 个环节（一、实验目的→二、实验设备与环境→三、实验要求→四、实验步骤与要点）。**注意：已废除“六、思考题”模块。**
2. `Exp_<编号>_<名称>_实验报告_学生模板.md` — 含 5 个环节（目的→设备→要求→**四、实验内容(要求留白)**→**五、实验结果分析(要求留白)**）。
  - **重要提醒**：在报告模板中，学生姓名、学号以及第四、五节内容必须为空白占位，绝不允许预填大纲答案。
3. **节标题格式检查**：普通实验必须使用中文序数（如 `实验一 XXXX`），若是期末综合实验则固定前缀为 `综合项目 XXXX`。
