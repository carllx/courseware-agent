...
## §7 词汇降维与价值锚定 (Sensory Metaphor & Value Anchoring)

> **来源**：教学口述风格的复现策略。解决学术术语（如 Encoding, Alignment）导致认知隔离的问题。

### 7.1 物理动词映射 (Physical Verb Mapping)
在解释抽象的数据映射或操作概念时，强制使用具有强烈物理感、肌肉感的动词，替代静态的学术描述词。
- ❌ **避免**：映射、分配、调整、改变
- ✅ **推荐**：翻译（"将数字用眼睛翻译成长度"）、按压（"按压在同一起跑线"）、劈开（"用空间劈开专属领地"）、挤压、堆叠

### 7.2 机械动作的价值锚定 (Value Anchoring for Mechanical Steps)
在讲解数据处理的机械步骤（如排序、对齐、过滤）时，必须追加一句该动作带来的**业务分析价值**。不要仅仅停留在"怎么做"，必须升华到"获得了什么洞察"。
- ❌ **仅描述动作**："我们通过排序，让柱子从高到低排列，这样看起来更清晰，引流视线。"
- ✅ **动作+价值锚定**："排序不仅是为了整齐，它是强行驱散数据的混乱，让无序的噪音自然结晶出'趋势'。而趋势，正是业务决策中最有价值的**洞察**。"
- ✅ **动作+价值锚定**："消除过度绘制，不仅是解决物理空间的拥挤，更是为了找回被黑压压的数据掩盖的**真相**。"
