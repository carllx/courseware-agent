...
### 4-1b Grid 多行 List 语法

> [VISUAL]
> *   **Slide**: S10b_Grid_Multiline_List
> *   **Layout**: `Grid`
> *   **Scene**: 四象限风险评估矩阵——高价值/低风险为绿色，高风险/低价值为红色
> **List**:
> - 高价值 × 低风险 (立即执行)
> - 高价值 × 高风险 (谨慎验证)
> - 低价值 × 低风险 (延后处理)
...
⚠️ **多行 List 陷阱**：每个 `- 项目` 行**必须**以 `>` 引用前缀开头（即 `> - 项目`），否则 VISUAL 块会在该行断裂，后续的 Asset 图片路径将丢失。
