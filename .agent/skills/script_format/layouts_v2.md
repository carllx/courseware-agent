...
| **`Comparison`** | 左右直接对抗式的优劣/选项对比 | Grid + compare theme |
...
| **List** | 可选 | 列表内容。**单行格式**：用 `/` 分隔项；**多行格式**：`**List**:` 后换行，每项 `> - 项目`（必须保留 `>` 前缀） |
...
```
*（上面是一个对比版式，系统读到 `Comparison` 宏，分配为 `Grid` 空间；读到 `List` 决定放入文本列表组合组件；分配红绿主题呈现）*
