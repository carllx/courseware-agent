...
> *   **Asset**: ![预览](../public/slides/S03_Marks_Channels_Intro.png)
> *   **Layout**: `Grid`
...
> 2. **线 (1D / Lines)**：线是有约束的。它的长度已经被它的存在本身"占用"了（用于连接起点和终点）。因此，你能调整的只剩下它的线宽以及它的颜色。在关系图或者折线图中，线是连接网络的主力。
> 3. **面 (2D / Areas)**：如果你画的是一个国家地图的版块，那它就是个面。它的尺寸和形状已经被地理边界死死绑定了。你几乎无法再去改变它的大小来代表其他含义，你能动用的只有它的填充颜色或纹理。
...
> *   **Asset**: ![预览](../public/slides/S03c_Five_Dim_Scatter.png)
> *   **Layout**: `Full`
> *   **Scene**: 一张精心渲染的五维散点图示意——X轴标注"年龄"，Y轴标注"收入"，图中散点的颜色按职业分类（蓝=IT，绿=金融），面积大小映射健康评分，形状区分性别（方=男，圆=女），右侧配以图例面板。
> *   **Text**: "五维数据，一张图搞定"
...
> 18 世纪末至 19 世纪初（1786-1801），英国统计学家 William Playfair 发明了柱状图和饼图，但当时并没有人真正意识到"位置通道"的生死攸关。直到 1854 年，伦敦医生 John Snow 在一张伦敦街道地图上，用小黑点标注了每一个霍乱死亡病例的位置——密集的黑点迅速聚集在 Broad Street 水泵周围。那张图直接说服了当局关闭了那口受污染的水泵，一场瘟疫就此被终结。这是人类历史上第一次，"空间位置通道"被用来拯救生命。
...
> *   **Asset**: ![预览](../public/slides/S04_Channel_Effectiveness.png)
> *   **Layout**: `Grid`
...
在信息可视化领域，有两位泰斗级人物 William S. Cleveland 和 Robert McGill，他们在 1980 年代做过一系列极为硬核的认知科学实验。他们让试验者辨认不同视觉通道中存在的微小数据差异。实验结果震惊了当时的图形学界：人类在判断不同维度的图形大小时，神经系统的敏感度呈现出了极为残酷和断崖式的层级划分！这就是为什么这些通道并不是平等的——它们在你的视网膜和前额叶皮层里，早就被漫长的生物进化安排好了"老幼尊卑"的铁定统领秩序。
...
> [VISUAL]
> *   **Slide**: `S04b_Expressiveness_Rule`
> *   **Asset**: ![预览](../public/slides/S04b_Expressiveness_Rule.png)
> *   **Layout**: `Split`
> *   **Scene**: 左半屏展示正确匹配——数量型数据（温度渐变）→量化通道（位置/长度），用绿色对勾标注；右半屏展示正确匹配——分类数据（水果种类）→标识通道（色相），用绿色对勾标注；中间用巨大的红色叉号标出错误的交叉搭配。
> *   **Text**: "表达性原则：数据特征必须精准匹配通道类型"

请大家看屏幕中间那个巨大的红色叉号：这就是用错了表达性原则的恐怖代价。当你手上拿到的是一堆汽车类别标签（福特、丰田、大众），你为了制造图表的"动态变化高级感"，非要强行按照序列给它们挂上从浅到深渐变的颜色亮度（这是属于量化通道）。你的读者脑海里瞬间就会产生一个荒诞的潜台词：“大众的颜色更深更粗犷，所以它的体量必然碾压福特？”这就完全扭曲了底层数据仅仅想展现分类标签的本意。这叫做“极其业余的通道越权”。

当然，很多同学会问：我明明可以用颜色区分种类，可不可以再顺便给它挂上不同的几何形状（比如蓝色圆圈、红色方块）？这叫**冗余编码（Redundant Encoding）**。双通道编码确实能极其有效且强烈地加强这批核心数据的视觉剥离感。但代价是什么呢？资源总是稀缺的。当你原本宝贵的通道资源（形状、颜色）被你一次性过度挥霍在这一个无关紧要的属性上时，这幅图就永远封死了加载第六维叙事数据的空间。

更为致命的是违背通道绑定特征的灾难。如果违反了这条法则，产生可笑的认知错乱——比如当你审视一张选举日战场的美国地图，如果用强烈的深红色阴影渐变去填充标注为共和党的广大荒芜内陆州，用微弱的粉色去区分为民主党的州地盘，读者视神经潜意识地就会判定"代表强烈颜色的那一方政治权力和胜率要远远强大百倍"，这种视觉谎言甚至会在敏感的选举周期中直接引发并扭曲舆论倒向。何况，**面积标记本身也是早已被地理边界死死硬绑定的产物**！你根本就不可能通过改变得克萨斯州的省份轮廓版图大小来任意强行映射选票数量的大小涨跌，一旦你试图动用缩放能力，整个国家的大陆地缘形状就会在读者眼里像被融化的塑胶玩具一样荒唐扭曲，这是不可理喻的视觉犯罪。

> [VISUAL]
> *   **Slide**: `S04c_Color_Lie_Map`
> *   **Asset**: ![预览](../public/slides/S04c_Color_Lie_Map.png)
> *   **Layout**: `Full`
> *   **Scene**: 一张被红色渐变严重误导的美国选举地图——共和党州用强烈的深红填充，民主党州用微弱的浅粉填充，旁边用黄色警告标签标注"视觉谎言：色相强度 ≠ 政治权力"，下方对比展示正确的分类色阶版本。
> *   **Text**: "当颜色变成武器——选举地图的视觉谎言"


而在有效性原则下，每一类通道都有着其不可撼动的阶级排名。也就是说，当你手上的数据是极其关键的绝对核心指标时，你必须把这笔预算花在最顶尖的通道上。
