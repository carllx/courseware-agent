...
def insert_before(filename, search_string, insert_string):
    ...
    if search_string in content:
        if insert_string.strip()[:20] in content:
            print(f"Already inserted in {filename}: {search_string[:10]}")
            return
        content = content.replace(search_string, "\n" + insert_string + "\n" + search_string)
        with open(path, "w", encoding="utf-8") as f:
            f.write(content)
        print(f"Success in {filename}: {search_string[:10]}")
    else:
        print(f"Failed to find target in {filename}: {search_string[:20]}...")
