...
![Figure 3.1 — Actions + Targets 全景映射图](../textbook/Visualization%20Analysis%20%26%20Design%20--%20Tamara%20Munzner%20--%202014/images/09771069f26c7f7a3c67a6b06d349512125f79a13bc20021f5d75bf5927f8c2d.webp)

### 3.3 动作的三层框架 (Three Levels of Actions)
用户目标（Actions）被分解为三个独立层级：分析（Analyze）、搜索（Search）和查询（Query）。

![Figure 3.2 — 动作的三层拆解：Analyze, Search, Query](../textbook/Visualization%20Analysis%20%26%20Design%20--%20Tamara%20Munzner%20--%202014/images/8338b3ad2ca06daea679c7d9deb0bee57a3b7eb45d2e7cd3b49bad11d916f37d.webp)
...
![Figure 3.6 — Targets 详细树状图（Trends, Outliers, Features; Attributes; Network; Spatial）](../textbook/Visualization%20Analysis%20%26%20Design%20--%20Tamara%20Munzner%20--%202014/images/c06e5416889462171602e0bb05ee392465a2be63288edf6eb3de281bf961409a.webp)
...
![Figure 3.10 — Strahler 数字派生过滤：保留 5000 个核心节点 vs 原始 50万节点](../textbook/Visualization%20Analysis%20%26%20Design%20--%20Tamara%20Munzner%20--%202014/images/5d354f57cd9d29339071b1edc9aa3a0f03de01a6f49b887106130bef700976eb.webp)

![Figure 3.5 — Derive 差值图经典案例：直接编码“贸易差额”而非分别画“进口”和“出口”](../textbook/Visualization%20Analysis%20%26%20Design%20--%20Tamara%20Munzner%20--%202014/images/673fa0bbb460f9b80c5ef88036f441827675459148f5c21c1e5a1d79b7f59da2.webp)
- **派生多属性 (流体力学空间)**：在原始 3D 物理空间视图中很难看清水流回旋区域，通过创建多个"派生空间视图"（如计算涡度与焓值的分布），特征就在派生视图中"聚集"起来，方便用户交互选择。

## 关键图表索引
- **Figure 3.1**: 动作与目标的全景映射图。左侧为动作（Actions），右侧为目标（Targets）。
- **Figure 3.2**: 动作的三层拆解：Analyze, Search, Query 的树状图象限。
- **Figure 3.5**: 派生（Derive）直接视觉编码的经典例子。与其画出两条曲线让用户肉眼费力比较高度差，不如直接计算出差值（如贸易顺差/逆差），绘制出直观的柱状图。
- **Figure 3.6**: 目标的详细树状图分解（Trends, Outliers, Features; Attributes 等）。
- **Figure 3.10 & 3.11**: 派生属性的极佳案例——使用 Strahler 数字提炼复杂树结构的骨架。
...
## 与逐字稿的对照检查表
- [ ] 是否向学生强调了 "Why doesn't dictate how"（意图不决定呈现形式，相同的目的可以有多种不同的设计方案）？
- [ ] 分析层：是否清楚讲解了 Discover / Present / Enjoy 三者的核心区别？
- [ ] 搜索层：是否用直白的语言（或类似找兔子、查字典的隐喻）解释了 Lookup/Locate/Browse/Explore 的两维象限？
- [ ] **关键点**：是否深度剖析了 **Derive (派生)** 的战略意义？（必须让 DMA 学生明白，遇到难以直接可视化的数据，第一步往往不是换图表，而是衍生新数据结构，比如 Figure 3.5 的差值图）。
