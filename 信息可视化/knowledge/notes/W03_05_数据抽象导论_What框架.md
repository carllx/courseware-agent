...
## 关键图表索引
- **Figure 2.1**: 何物可以被可视化的抽象类型全景图（What: Data Abstraction 树形图）。
- **Figure 2.2**: 五种基本数据类型（Items, Attributes, Links, Positions, Grids）的图解。
- **Figure 2.3 & 2.4**: 数据集类型如何由数据类型组成，以及表格、网络、场、几何的具体结构图解。重点理解单元格（Cell）在表格和场中的不同构成。
- **Figure 2.6**: 静态文件与动态数据流的图解对比。
...
## 与逐字稿的对照检查表
- [ ] 是否向学生清晰定义了什么是 "Item", "Attribute", "Link"（避开复杂的 Grid 和 Position 如果涉及不深）？
- [ ] 是否说明了表格数据（Tables）和网络/树（Networks/Trees）的根本区别？
- [ ] 是否传达了元数据（Metadata）对于理解数字含义（Semantics）的重要性（用类似 Figure 2.1 的纯数字 vs 带表头数据的例子）？
- [ ] （可选，根据 M01/02 深度）是否澄清了 Continuous (场) 和 Discrete (表格/网络) 数据集在插值上的区别？
