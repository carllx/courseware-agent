...
- [ ] `CHK-B02-01`: 是否向学生强调了 "Categorical" (分类) 与 "Quantitative" (定量) 数据的核心区别（决定了图形通道能否使用大小/长度）？
  - 关键词: `分类`, `定量`, `Categorical`, `Quantitative`, `有序`
  - 预期出现模块: M02
- [ ] `CHK-B02-02`: 是否清晰解释了 "Key" 和 "Value" 的概念？（用学生熟悉的 Excel 列、或者数据库概念、或者自变量/因变量隐喻）
  - 关键词: `键`, `值`, `Key`, `Value`, `自变量`, `因变量`
  - 预期出现模块: M02
- [ ] `CHK-B02-03`: 是否说明了把连续定量数据聚合（Aggregate）为低精度分类数据（如温度数字 -> 冷/暖/热）是可视化设计的常见策略？
  - 关键词: `聚合`, `分箱`, `Aggregate`, `Bin`
  - 预期出现模块: M02 或 M04
- [ ] `CHK-B02-04`: 是否梳理了时间的特殊性（既可以是循环的 Cyclic、也可以是顺序的 Sequential；既可以是 Key，也可以是 Value）？
  - 关键词: `时间`, `循环`, `Cyclic`, `时间序列`, `Time-series`
  - 预期出现模块: M02
