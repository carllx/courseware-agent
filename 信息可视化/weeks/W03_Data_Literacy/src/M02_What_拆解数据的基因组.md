...
> [LIFE CONNECT]
> 各位习惯了用画布和画笔思考，当我们要画一棵树，你会观察它的光影和色彩。

> [WARNING]
> 但当面对包含十万行记录的“全球森林分布表”时，你的美术直觉会瞬间瘫痪。因为数据可视化不画“肉眼看到的树”，它画的是数字的“基因”。如果我们不能把庞大的表格劈开、拆成机器能理解的最小颗粒，再强大的设计软件也画不出一根图钉。

> [TECH NOTE]
拿到杂乱无章的数据后，我们必须强制自己剥离视觉表象，理清它的底层结构（What）。在发号施令前，必须明白我们在处理什么类型的数据。
