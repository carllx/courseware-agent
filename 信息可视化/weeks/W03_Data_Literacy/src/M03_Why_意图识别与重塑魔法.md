...
用户盯着屏幕的时候，他是仅仅被动地"看数据"，还是准备主动"创造新洞察"？Munzner 将 Analyze（大方向）拆分为两大阵营：被动吸收的**消费型（Consume）**与主动干预的**创生型（Produce）**。

首先来看消费型（Consume），它包含三种不同的看图心态，直接决定了你的设计基调：

> [VISUAL]
> *   **Slide**: `S26_Analyze_Layer`
> *   **Layout**: `Grid`
> *   **Scene**: 三张面具。第一张：宣威者举着金牌大声夸耀；第二张：侦探在非常黑暗的洞窟内用放大镜搜索；第三张：游客带着耳机沉醉浅笑享受。
> *   **Text**: "消费型面具：汇报、探索者、享受者"
> *   **List**: 
>     - 宣示展示
>     - 幽暗发现
>     - 消遣享受
...
> *   **Q**: 当你打开一张全景式的《全国百强县经济热力地图》时，你并没有打算查找你的老家（目标不明确），也没有特定的比对意图，只是凭借直觉，在平滑缩放的地图画布上漫游，想看看哪里的颜色最深、经济最强。根据 Munzner 的 Search 意图矩阵，这种寻猎行为最符合以下哪种模式？
> *   **Options**: A. Lookup（明确检索）：依赖精准文本输入框直接定位特定条目 | B. Locate（迷雾定位）：已知目标特征，在全局迷雾中通过高亮悬停手段追踪 | C. Browse（漫游扫描）：无明确指定目标，凭借直觉在平滑缩放或滚动的视场中扫荡特征 | D. Explore（混沌探索）：对目标特征与位置完全未知，需要动用多图联动或套索框选深层排查
> *   **Answer**: `C`
> *   **Explain**: 此场景下用户"无确切指定目标"（排除需要精准靶心的 Lookup 和 Locate），但大脑带有潜意识的直觉特征偏好（寻找深色区域），只需平滑拖拽或缩放（Semantic Zooming），不需要套索框选等多重联动探明工具（排除 Explore）。这完全符合 Browse 模式像老鹰巡视领地般的特征定义。
