...
> [CASE STUDY]
> 让我分享一个上届的真实案例。在 **One Pager（一页纸项目提案）互评阶段**，有一个小组的提案收到了诸多“效果肯定很震撼”的赞美。然而，**没有任何人指出**其中的一个基础逻辑错误：他们试图把**离散的分类数据（Categorical Data，比如苹果、橘子、香蕉这种没有高低之分的类别）**，**强行拉伸成连续的 3D 高度（Height Channel，比如柱子的高低长短）**。
> 
...
> *   **List**: 数据锚定 / 嵌套模型 / 数据断层
> *   **Asset**: ![预览](../public/slides/m06_s18_2_critique_diagram.png)

作为设计者，大家在互评时需要执行以下**三大任务**：

**第一，审查视觉元素是否有“数据锚定”（即：画出的每个视觉特征，能不能像钉子一样死死钉在对应的数据列上）。**
很多 One Pager 充斥着“酷炫的滤镜”、“科技感光环”等冗余形容词。依据**郝亚维老师的《信息可视化设计》工作流**，第一步必须是**建立信息逻辑**。互评时，请拿着红笔指着草图上的那颗发光星球问：“**它的半径死抠的是哪一列数据？它的光晕强度绑定的是哪一列数据？**”如果对方答不上来，或者数据表里根本没有能决定大小和颜色的数值列，这就是毫无意义的**视觉噪音**，会直接导致**“认知负荷过高”**。请划掉并铲除这些无数据支撑的表述。
...
依据 **Tamara Munzner 的 Why-What-How 渐进分析模型**，常见的问题是**跨层跳跃**。例如，**任务与数据抽象层（Why 和 What，即食客想吃什么口味、手里有什么菜）**还未理清，就直接跨越到了**视觉编码层（How，你要怎么摆盘）**。对于设计专业学生来说，最常见的通病就是：**连用户是要看“整体趋势”还是“对比极端值”（Why）都没定下来，组员们就开始为了该用“赛博朋克粉”还是“克莱因蓝”（How）吵得不可开交！**互评时请核对：当前的**数据特征（What，你的菜）**和**用户任务（Why，食客想吃什么口味）**，是否足以支撑其计划的**视觉表达（How，摆成什么样）**。如果不匹配，必须立刻推翻重构。
...
> *   **Layout**: `Center`
> *   **Text**: "互评执行：下游验证"
> *   **List**: 1.交换提案 / 2.标注逻辑跳跃 / 3.标记视觉噪音 / 4.核对数据字段
...
现在，大家交换提案。我们要站在开发者的视角执行**“下游验证”**（即：把自己当成即将接手这套设计的程序员，假装现在就要开始敲代码，看看图上的元素能不能在真实数据中找到落脚点）。请仔细审阅对方的设计：一旦发现跨层跳跃、视觉噪音或是缺乏数据支撑的元素，请直接做出标记。
